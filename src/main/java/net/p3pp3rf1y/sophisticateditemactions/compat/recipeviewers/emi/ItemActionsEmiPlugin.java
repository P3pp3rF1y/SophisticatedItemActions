...
public class ItemActionsEmiPlugin implements EmiPlugin {
	...
	private static final ResourceLocation RESTOCK_BUTTON_TEXTURE = SophisticatedItemActions.getRL("textures/gui/emi_restock_button.png");
	...
	private static List<List<ItemStack>> getIngredientOptions(EmiRecipe recipe, boolean fullStacks) {
		...
		for (EmiIngredient ingredient : recipe.getInputs()) {
			...
			for (EmiStack emiStack : ingredient.getEmiStacks()) {
				...
				if (!stack.isEmpty() && options.stream().noneMatch(option -> ItemStack.isSameItemSameTags(option, stack))) {
	...
	private static class RecipeRestockWidget extends Widget {
		...
		public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
			int textureY = bounds.contains(mouseX, mouseY) ? RESTOCK_BUTTON_SIZE : 0;
			guiGraphics.blit(RESTOCK_BUTTON_TEXTURE, bounds.x(), bounds.y(), 0, textureY, RESTOCK_BUTTON_SIZE, RESTOCK_BUTTON_SIZE, RESTOCK_BUTTON_SIZE,
					RESTOCK_BUTTON_SIZE * 2);
		}
		...
		private Optional<ResourceLocation> getRegisteredRecipeId() {
			...
			return ClientRecipeHelper.getRecipe(backingRecipe.getId()).filter(registeredRecipe -> registeredRecipe == backingRecipe).map(Recipe::getId);
		}
	}
}
