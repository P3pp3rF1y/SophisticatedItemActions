	...
	private static class RecipeRestockWidget extends WidgetWithBounds {
		...
		public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
			...
			if (bounds.contains(mouseX, mouseY)) {
				RenderSystem.setShaderColor(1, 1, 160 / 255F, 1);
			}
		...
		private static Optional<ResourceLocation> getRegisteredRecipeId(Display display) {
			...
			return craftingDisplay.getOptionalRecipe()
					.flatMap(recipe -> ClientRecipeHelper.getRecipe(recipe.getId()).filter(registeredRecipe -> registeredRecipe == recipe).map(Recipe::getId));
		}
	}
}
