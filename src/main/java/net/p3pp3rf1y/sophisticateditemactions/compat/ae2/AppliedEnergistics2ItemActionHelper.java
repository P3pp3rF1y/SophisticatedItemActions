...
public final class AppliedEnergistics2ItemActionHelper {
	...
	public static <T extends ITerminalHost & IActionHost> int insertItem(ServerPlayer player, ItemStack stack, T host) {
		MEStorage storage = host.getInventory();
	...
	public static <T extends ITerminalHost & IActionHost> int extractItem(ServerPlayer player, ItemStack stack, T host) {
		MEStorage storage = host.getInventory();
		AEItemKey key = AEItemKey.of(stack);
		if (storage == null || key == null || !host.getLinkStatus().connected()) {
			return 0;
