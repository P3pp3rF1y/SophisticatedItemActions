...
public class RenderedEntityBlockHighlightRenderer {
	...
	public static void render(PoseStack poseStack, float partialTick, Vec3 cameraPos) {
		...
		highlightedPositions.forEach((color, entityHighlightData) -> entityHighlightData.forEach(highlightData -> {
			...
			if (cachedHighlight != null) {
				renderHighlightedBlock(submitNodeCollector, poseStack, partialTick, cameraPos, cachedHighlight, mc, color);
			}
		}));
	}
	...
	private static void renderHighlightedBlock(SubmitNodeCollector submitNodeCollector, PoseStack poseStack, float partialTick, Vec3 cameraPos,
			CachedEntityHighlight cachedHighlight, Minecraft mc, int color) {
