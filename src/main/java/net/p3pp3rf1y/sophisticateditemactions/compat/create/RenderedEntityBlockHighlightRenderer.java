...
public class RenderedEntityBlockHighlightRenderer {
	...
	public static void render(PoseStack poseStack, float partialTick, Vec3 cameraPos) {
		...
		highlightedPositions.forEach((color, entityHighlightData) -> entityHighlightData.forEach(highlightData -> {
			...
			if (cachedHighlight != null) {
				submitHighlightedBlock(submitNodeStorage, poseStack, partialTick, cameraPos, cachedHighlight, mc, buffer, color);
			}
		}));
	}
	...
	private static void submitHighlightedBlock(SubmitNodeCollector submitNodeCollector, PoseStack poseStack, float partialTick, Vec3 cameraPos,
			CachedEntityHighlight cachedHighlight, Minecraft mc, MultiBufferSource.BufferSource buffer, int color) {
