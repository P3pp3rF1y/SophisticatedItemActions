...
public class ItemFlightAnimator {
	...
	public static void submitItems(SubmitNodeCollector submitNodeCollector, PoseStack poseStack, float partialTick, Vec3 cameraPos) {
		...
		while (it.hasNext()) {
			...
			itemStackRenderState.submit(poseStack, submitNodeCollector,
					LightCoordsUtil.getLightCoords(mc.level, new BlockPos((int) pos.x, (int) pos.y, (int) pos.z)), OverlayTexture.NO_OVERLAY, 0);
