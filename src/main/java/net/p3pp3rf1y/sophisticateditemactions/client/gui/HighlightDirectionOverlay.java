...
public class HighlightDirectionOverlay {
	private static final Identifier CONTROLS = SophisticatedItemActions.getIdentifier("textures/gui/controls.png");
	...
	private static void render(GuiGraphicsExtractor guiGraphics, net.minecraft.client.DeltaTracker deltaTracker) {
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, CONTROLS, x, y, 0, 0, LINE_WIDTH, LINE_HEIGHT, TEXTURE_WIDTH, TEXTURE_HEIGHT, getColor(0xFFFFFF, visibilityAlpha));

		for (Dot dot : groupDots(dots)) {
			int dotX = x + Math.round((dot.normalizedPosition() + 1F) * (LINE_WIDTH - DOT_SIZE) / 2F);
			guiGraphics.blit(RenderPipelines.GUI_TEXTURED, CONTROLS, dotX, y, 0, DOT_V, DOT_SIZE, DOT_SIZE, TEXTURE_WIDTH, TEXTURE_HEIGHT, getColor(dot.color(), visibilityAlpha));
		}
	...
	private static void renderDistance(GuiGraphicsExtractor guiGraphics, Font font, int lineX, int lineY, Dot closest) {
		...
		guiGraphics.text(font, distanceText, textX, lineY - 10, getColor(closest.color(), visibilityAlpha), true);
	}
