...
public class HighlightDirectionOverlay {
	...
	public static final IGuiOverlay HUD_HIGHLIGHT_DIRECTIONS = (gui, guiGraphics, partialTicks, width, height) -> render(guiGraphics, width, height);
	...
	private static void render(GuiGraphics guiGraphics, int width, int height) {
		...
		int y = height - HUD_Y_OFFSET;
		RenderSystem.enableBlend();
		RenderSystem.defaultBlendFunc();
		RenderSystem.setShaderColor(1, 1, 1, visibilityAlpha);
		guiGraphics.blit(CONTROLS, x, y, 0, 0, LINE_WIDTH, LINE_HEIGHT, TEXTURE_WIDTH, TEXTURE_HEIGHT);
		RenderSystem.setShaderColor(1, 1, 1, 1);

		for (Dot dot : groupDots(dots)) {
			int dotX = x + Math.round((dot.normalizedPosition() + 1F) * (LINE_WIDTH - DOT_SIZE) / 2F);
			setShaderColor(dot.color(), visibilityAlpha);
			guiGraphics.blit(CONTROLS, dotX, y, 0, DOT_V, DOT_SIZE, DOT_SIZE, TEXTURE_WIDTH, TEXTURE_HEIGHT);
			RenderSystem.setShaderColor(1, 1, 1, 1);
		}
		RenderSystem.disableBlend();

		dots.stream().min(Comparator.comparingDouble(Dot::distance)).ifPresent(closest -> renderDistance(guiGraphics, mc.font, x, y, width, closest));
	}
	...
	private static void renderDistance(GuiGraphics guiGraphics, Font font, int lineX, int lineY, int width, Dot closest) {
		...
		int textX = Mth.clamp(dotCenterX - font.width(distanceText) / 2, 0, width - font.width(distanceText));
		guiGraphics.drawString(font, distanceText, textX, lineY - 10, Math.round(visibilityAlpha * 255) << 24 | closest.color(), true);
	}

	private static void setShaderColor(int color, float alpha) {
		RenderSystem.setShaderColor((color >> 16 & 255) / 255F, (color >> 8 & 255) / 255F, (color & 255) / 255F, alpha);
	}
