...
public class ClientEventHandler {
	...
	private static void submitCustomGeometry(SubmitCustomGeometryEvent event) {
		...
		RenderedBlockHighlightRenderer.render(event.getSubmitNodeCollector(), event.getPoseStack(), partialTick,
				event.getLevelRenderState().cameraRenderState.pos);
		ClientCompatRenderHelper.renderCustomGeometry(event.getSubmitNodeCollector(), event.getPoseStack(), partialTick,
				event.getLevelRenderState().cameraRenderState.pos);
	...
	public static boolean tryHighlightGuiItem(ItemStack stack) {
		...
		if (tryHighlightItem(player, stack)) {
			guiHighlightRequestsPendingResult++;
			guiHighlightRequestScreen = Minecraft.getInstance().gui.screen();
		}
		return true;
	}
	...
	public static void handleHighlightResult(boolean hasMatches) {
		...
		guiHighlightRequestsPendingResult--;
		if (hasMatches && Minecraft.getInstance().gui.screen() == guiHighlightRequestScreen) {
