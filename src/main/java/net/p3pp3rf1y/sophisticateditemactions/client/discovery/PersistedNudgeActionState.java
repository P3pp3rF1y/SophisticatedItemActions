...
public class PersistedNudgeActionState {
	...
	public static PersistedNudgeActionState fromTag(CompoundTag tag) {
		...
		state.successfulUseCount = tag.getInt(SUCCESSFUL_USE_COUNT_TAG).orElse(0);
		if (tag.contains(LAST_SUCCESSFUL_USE_PLAY_TIME_TAG)) {
			state.lastSuccessfulUsePlayTime = tag.getLong(LAST_SUCCESSFUL_USE_PLAY_TIME_TAG).orElse(Long.MIN_VALUE);
		}
		if (tag.contains(LAST_SHOWN_PLAY_TIME_TAG)) {
			state.lastShownPlayTime = tag.getLong(LAST_SHOWN_PLAY_TIME_TAG).orElse(Long.MIN_VALUE);
		}
		return state;
	}
}
