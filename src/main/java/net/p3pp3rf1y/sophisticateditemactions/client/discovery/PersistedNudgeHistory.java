...
public class PersistedNudgeHistory {
	...
	public static PersistedNudgeHistory fromTag(CompoundTag tag) {
		...
		for (NudgeHintType hintType : NudgeHintType.values()) {
			if (tag.contains(hintType.name())) {
				tag.getCompound(hintType.name()).ifPresent(compoundTag -> history.actionStates.put(hintType, PersistedNudgeActionState.fromTag(compoundTag)));
			}
		}
		return history;
	}
