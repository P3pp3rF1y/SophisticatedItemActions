...
public interface ISubLevelCompat {
	...
	List<BlockEntity> getBlockEntitiesInRange(Level level, BlockPos origin, int range);
	...
	default boolean mayInteract(Player player, Level level, BlockPos pos) {
		return !(level instanceof ServerLevel serverLevel) || player.mayInteract(serverLevel, pos);
	}
