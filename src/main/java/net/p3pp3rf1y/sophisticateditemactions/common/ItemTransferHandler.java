...
public class ItemTransferHandler {
	...
	@Nullable
	public static RestockRegisteredRecipePayload createRestockRegisteredRecipePayload(Player player, Identifier recipeId, boolean fullStacks) {
		Map<Identifier, List<BlockPos>> storages = getInteractionStoragePositionsAround(player);
		Map<Identifier, List<Integer>> entities = getStorageEntitiesAround(player);
		boolean hasRecipeInventorySource = ItemTransferExtensionRegistry.getExtension().map(extension -> extension.hasRecipeInventorySource(player))
				.orElse(false);
		if (!storages.isEmpty() || !entities.isEmpty() || hasRecipeInventorySource) {
			return new RestockRegisteredRecipePayload(recipeId, fullStacks, storages, entities);
		}
		playError(player, ItemActionsTranslationHelper.INSTANCE.translStatusMessage("no_storage_in_range").setStyle(Style.EMPTY.withColor(0xFF5555)));
		return null;
	}
	...
	private static void showAlternativeRestockMessage(Player player, int minSlot, int maxSlot, boolean fillEmpty, ItemStack filter,
		...
		if (restockedPlayerSlots.isEmpty()) {
			if (maxSlot - minSlot == 1) {
				ItemStack item = fillEmpty ? filter : player.getInventory().getItem(minSlot);
				player.sendOverlayMessage(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("cannot_restock_item",
						Component.literal(item.getHoverName().getString()).withStyle(ChatFormatting.RED)));
			} else {
				player.sendOverlayMessage(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("cannot_restock_items"));
			}
	...
	public static void handleRecipeRestock(Player player, Map<Identifier, List<BlockPos>> storagePositions, Map<Identifier, List<Integer>> entities,
		...
		RecipeHelper.getRecipe(recipeId)
				.map(recipe -> getRecipeIngredientOptions(RecipeHelper.getIngredients(recipe.value()).stream().flatMap(Optional::stream).toList(), fullStacks))
	...
	static List<List<ItemStack>> getMissingRecipeIngredientOptions(List<List<ItemStack>> ingredientOptions, List<ItemStack> inventory) {
		...
		for (List<ItemStack> options : ingredientOptions) {
			...
			int remaining = normalizedOptions.getFirst().getCount();
			while (remaining > 0) {
				List<ItemStack> matchingTarget = targets.stream().filter(
						target -> hasSameIngredientOptions(target, normalizedOptions) && target.getFirst().getCount() < target.getFirst().getMaxStackSize())
						.findFirst().orElse(null);
				if (matchingTarget == null) {
					int count = Math.min(remaining, normalizedOptions.getFirst().getMaxStackSize());
					targets.add(normalizedOptions.stream().map(stack -> stack.copyWithCount(count)).toList());
					remaining -= count;
					continue;
				}

				int added = Math.min(remaining, matchingTarget.getFirst().getMaxStackSize() - matchingTarget.getFirst().getCount());
				matchingTarget.forEach(stack -> stack.grow(added));
				remaining -= added;
			}
		}
