...
import net.p3pp3rf1y.sophisticateditemactions.network.RestockAlternativeItemsMessage;
import net.p3pp3rf1y.sophisticateditemactions.network.RestockItemsMessage;
import net.p3pp3rf1y.sophisticateditemactions.network.RestockRecipeItemsMessage;
import net.p3pp3rf1y.sophisticateditemactions.network.RestockRegisteredRecipeMessage;
...
public class ItemTransferHandler {
	...
	private static Map<ResourceLocation, List<BlockPos>> getInteractionStoragePositionsAround(Player player) {
		...
		WorldHelper.getBlockEntitiesInRange(level, player.blockPosition(), INTERACTION_RANGE).forEach(be -> {
			ItemActionHandlerRegistry.getBlockHandlerFor(level, be.getBlockPos(), be, IBlockItemActionHandler.Action.DEPOSIT).ifPresent(handler -> {
				tempStorages.computeIfAbsent(handler.id(), k -> new HashSet<>())
						.add(handler.getInteractionPosToActOn(level, be.getBlockPos(), be, IBlockItemActionHandler.Action.DEPOSIT));
			});
		});
	...
	private static Map<ResourceLocation, List<Integer>> getStorageEntitiesAround(Player player) {
		...
		player.level().getEntities(player, player.getBoundingBox().inflate(INTERACTION_RANGE), e -> e.distanceTo(player) <= INTERACTION_RANGE).forEach(
				e -> ItemActionHandlerRegistry.getEntityHandlerIdFor(e).ifPresent(id -> entities.computeIfAbsent(id, k -> new ArrayList<>()).add(e.getId())));
		return entities;
	}
	...
	public static void handleDeposit(Player player, int minSlot, int maxSlot, Map<ResourceLocation, List<BlockPos>> storagePositions,
		...
		for (int slot = minSlot; slot < maxSlot; slot++) {
			...
			for (IDepositHandler depositHandler : handlers) {
				...
				if (match == ItemMatchResult.MATCHING_STACK) {
					stack = depositToHandlerAndLog(depositHandler, stack, player, slot, inserted, depositedFromSlots, depositedStacks);
					if (stack.isEmpty()) {
						break;
					}
				} else {
		...
		int extensionDepositedStacks = ItemTransferExtensionRegistry.getExtension().map(extension -> extension.depositFromInventorySources(player, minSlot,
				maxSlot, onlyMatching, collectStorageItemHandlerTargets(player, storagePositions, entities, serverPlayer), inserted)).orElse(0);

		Vec3 playerPos = player.getEyePosition().add(0, -0.1, 0);
		List<ItemTransferData> itemTransferData = inserted.values().stream().toList();
		ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer, new SyncItemTransfersMessage(itemTransferData, playerPos, true, false));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, true, false), serverPlayer);
	...
	private static void showDepositMessage(Player player, int minSlot, int maxSlot, Set<Integer> depositedFromSlots, List<ItemStack> depositedStacks,
		...
		if (depositedFromSlots.isEmpty() && extensionDepositedStacks == 0) {
			...
			if (extensionDepositedStacks > 0) {
				message = ItemTransferExtensionRegistry.getExtension()
						.map(extension -> extension.getDepositMessage(depositedFromSlots.size(), extensionDepositedStacks)).orElse(message);
			} else if (!depositedFromSlots.isEmpty()) {
				if (maxSlot - minSlot == 1) {
					message = message.copy().append(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("deposited_item",
							Component.literal(depositedStacks.get(0).getHoverName().getString()).withStyle(ChatFormatting.DARK_GREEN)));
				} else {
					message = message.copy().append(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("deposited_items",
	...
	private static List<StorageItemHandlerTarget> collectStorageItemHandlerTargets(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions,
	...
	private static ItemStack depositToHandlerAndLog(IDepositHandler depositHandler, ItemStack stack, Player player, int slot,
			Map<Vec3, ItemTransferData> inserted, Set<Integer> depositedFromSlots, List<ItemStack> depositedStacks) {
		ItemStack remaining = depositHandler.insertItem(stack);
		if (remaining.getCount() < stack.getCount()) {
			ItemStack transferredStack = stack.copyWithCount(stack.getCount() - remaining.getCount());
	...
	public static void restockRecipeItems(Player player, List<List<ItemStack>> ingredientOptions) {
		...
		if (!storages.isEmpty() || !entities.isEmpty() || hasRecipeInventorySource) {
			ItemActionsPacketHandler.INSTANCE.sendToServer(new RestockRecipeItemsMessage(ingredientOptions, storages, entities));
		} else {
			playError(player, ItemActionsTranslationHelper.INSTANCE.translStatusMessage("no_storage_in_range").setStyle(Style.EMPTY.withColor(0xFF5555)));
		}
	}
	...
	public static void restockRecipeItems(Player player, ResourceLocation recipeId, boolean fullStacks) {
		...
		boolean hasRecipeInventorySource = ItemTransferExtensionRegistry.getExtension().map(extension -> extension.hasRecipeInventorySource(player))
				.orElse(false);
		if (!storages.isEmpty() || !entities.isEmpty() || hasRecipeInventorySource) {
			ItemActionsPacketHandler.INSTANCE.sendToServer(new RestockRegisteredRecipeMessage(recipeId, fullStacks, storages, entities));
		} else {
			playError(player, ItemActionsTranslationHelper.INSTANCE.translStatusMessage("no_storage_in_range").setStyle(Style.EMPTY.withColor(0xFF5555)));
		}
	}
	...
	private static void restockAlternativeItems(Player player, List<ItemStack> filters, int minSlot, int maxSlot, boolean fillEmpty, boolean refillSingle) {
		...
		Map<ResourceLocation, List<Integer>> entities = getStorageEntitiesAround(player);
		if (!storages.isEmpty() || !entities.isEmpty()) {
			ItemActionsPacketHandler.INSTANCE
					.sendToServer(new RestockAlternativeItemsMessage(filters, minSlot, maxSlot, fillEmpty, refillSingle, storages, entities));
		} else {
			playError(player, ItemActionsTranslationHelper.INSTANCE.translStatusMessage("no_storage_in_range").setStyle(Style.EMPTY.withColor(0xFF5555)));
		}
	}
	...
	public static void handleRestock(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions, Map<ResourceLocation, List<Integer>> entities,
		...
		ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer, new SyncItemTransfersMessage(itemTransferData, playerPos, false, false));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, false, false), serverPlayer);
		...
		if (restockedPlayerSlots.isEmpty() && extensionRestockedStacks == 0) {
			...
			if (extensionRestockedStacks > 0) {
				message = ItemTransferExtensionRegistry.getExtension()
						.map(extension -> extension.getRestockMessage(restockedPlayerSlots.size(), extensionRestockedStacks)).orElse(message);
			} else if (!restockedPlayerSlots.isEmpty()) {
				if (maxSlot - minSlot == 1) {
					message = message.copy().append(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("restocked_item",
							Component.literal(restockedStacks.get(0).getHoverName().getString()).withStyle(ChatFormatting.DARK_GREEN)));
				} else {
					message = message.copy().append(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("restocked_items",
	...
	private static void syncRestockTransfers(ServerPlayer player, Map<Vec3, ItemTransferData> restocked) {
		Vec3 playerPos = player.getEyePosition().add(0, -0.3, 0);
		List<ItemTransferData> itemTransferData = restocked.values().stream().toList();
		ItemActionsPacketHandler.INSTANCE.sendToClient(player, new SyncItemTransfersMessage(itemTransferData, playerPos, false, false));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, false, false), player);
	}
	...
	private static void showAlternativeRestockMessage(Player player, int minSlot, int maxSlot, boolean fillEmpty, ItemStack filter,
		...
		if (restockedPlayerSlots.isEmpty()) {
			...
			player.playNotifySound(SoundEvents.NOTE_BLOCK_BASS.value(), SoundSource.PLAYERS, 1,
					0.7f + RandHelper.getRandomMinusOneToOne(player.level().random) * 0.1F);
			return;
		}

		if (maxSlot - minSlot == 1) {
			player.displayClientMessage(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("restocked_item",
					Component.literal(restockedStacks.get(0).getHoverName().getString()).withStyle(ChatFormatting.DARK_GREEN)), true);
		} else {
	...
	public static void handleAlternativeRestock(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions,
			Map<ResourceLocation, List<Integer>> entities, List<ItemStack> filters, int minSlot, int maxSlot, boolean fillEmpty, boolean refillSingle) {
		...
		showAlternativeRestockMessage(player, minSlot, maxSlot, fillEmpty, filters.get(0), restockedPlayerSlots, restockedStacks);
	}
	...
	public static void handleRecipeRestock(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions, Map<ResourceLocation, List<Integer>> entities,
		...
		if (missingIngredientOptions.isEmpty()) {
			if (backpackRestockedCount > 0) {
				player.displayClientMessage(ItemActionsTranslationHelper.INSTANCE.translStatusMessage("restocked_recipe_items",
						Component.literal(String.valueOf(backpackRestockedCount)).withStyle(ChatFormatting.DARK_GREEN)), true);
				Vec3 playerPos = player.getEyePosition().add(0, -0.3, 0);
				ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer, new SyncItemTransfersMessage(List.of(), playerPos, false, true));
			} else {
		...
		ItemActionsPacketHandler.INSTANCE.sendToClient(serverPlayer,
				new SyncItemTransfersMessage(itemTransferData, playerPos, false, !restockedPlayerSlots.isEmpty() || backpackRestockedCount > 0));
		ItemActionsPacketHandler.INSTANCE.sendToAllTracking(new SyncItemTransfersMessage(itemTransferData, playerPos, false, false), serverPlayer);
	...
	public static void handleRecipeRestock(Player player, Map<ResourceLocation, List<BlockPos>> storagePositions, Map<ResourceLocation, List<Integer>> entities,
		...
		RecipeHelper.getRecipe(recipeId).map(recipe -> getRecipeIngredientOptions(recipe.getIngredients(), fullStacks))
	...
	static List<List<ItemStack>> getRecipeIngredientOptions(List<Ingredient> ingredients, boolean fullStacks) {
		return ingredients.stream().map(Ingredient::getItems)
				.map(options -> Arrays.stream(options).filter(stack -> !stack.isEmpty())
	...
	static List<List<ItemStack>> getMissingRecipeIngredientOptions(List<List<ItemStack>> ingredientOptions, List<ItemStack> inventory) {
		...
		for (List<ItemStack> options : ingredientOptions) {
			...
			if (matchingTarget == null) {
				...
				int count = (int) Math.min((long) matchingTarget.get(0).getCount() + normalizedOptions.get(0).getCount(),
						36L * matchingTarget.get(0).getMaxStackSize());
		...
		for (List<ItemStack> options : targets) {
			int remaining = options.get(0).getCount();
	...
	private static boolean hasSameIngredientOptions(List<ItemStack> first, List<ItemStack> second) {
		...
		for (int index = 0; index < first.size(); index++) {
			if (!ItemStack.isSameItemSameTags(first.get(index), second.get(index))) {
				return false;
			}
		}
		return true;
	...
	private static List<ItemStack> normalizeIngredientOptions(List<ItemStack> options) {
		...
		for (ItemStack option : options) {
			...
			List<ItemStack> optionsWithSameHash = optionsByHash.computeIfAbsent(option.getItem().hashCode(), key -> new ArrayList<>());
			if (optionsWithSameHash.stream().noneMatch(existing -> ItemStack.isSameItemSameTags(existing, option))) {
	...
	private static void restockRecipeIngredient(List<IRestockHandler> restockHandlers, List<ItemStack> ingredientOptions, Player player,
		...
		int remaining = ingredientOptions.get(0).getCount();
	...
	private static boolean matchesAnyFilter(ItemStack stack, List<ItemStack> filters) {
		return !stack.isEmpty() && filters.stream().anyMatch(filter -> ItemStack.isSameItemSameTags(stack, filter));
	}
		...
		for (IRestockHandler handler : restockHandlers) {
			...
			if (!transfers.isEmpty()) {
				for (IRestockHandler.RestockTransfer transfer : transfers) {
					...
					if (playerInventoryStack.isEmpty()) {
						playerInventoryStack = transferredStack.copy();
					} else {
						playerInventoryStack.grow(transferredStack.getCount());
					}
				...
				stackToExtract = stackToExtract
						.copyWithCount(Math.max(0, stackToExtract.getCount() - transfers.stream().mapToInt(transfer -> transfer.stack().getCount()).sum()));
			}
